...
public class Manager : MonoBehaviour {
    ...
    private int[] layers = new int[] { 1, 25, 25, 1 }; //1 input and 1 output
	...
	void Update ()
    {
        if (isTraning == false)
        {
            ...
            {
                nets.Sort();
                for (int i = 0; i < populationSize / 2; i++) //0-25
                {
                    nets[i] = new NeuralNetwork(nets[i+(populationSize / 2)]); //0-25 -> 25-50
